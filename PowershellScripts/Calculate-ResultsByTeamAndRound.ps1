param (
    ...
    [string] $season = '2011',
    [string] $powershellScriptsFolder = 'fl:\PowershellScripts'
)

[string] $inputMatchesFile = "$matchResultsFolder\Super_rugby_$($season).csv"
[string] $outputFolder = "$matchResultsFolder\$($season)"
...
. "$powershellScriptsFolder\Create-UtilityFunctions.ps1"

$allPlayerStats = import-csv "$playerStatsFolder\AllPlayerStats.csv"
...
$allResults = $matchResults | % {
    ...
    if ($matchResult.Round -ne $currentRound)
    {
        ...
        $playerStatsForRoundFileName = "$playerStatsFolder\ByRound\Round_$($currentRound).csv"
    ...
    $homeTeamResult = $matchResult | select-object Round,DatePlayed, `
    ...
    $awayTeamResult = $matchResult | select-object Round,DatePlayed, `
...
$allResults | export-csv "$matchResultsFolder\$season\AllResultsByTeamAndRound.csv" -noTypeInformation
