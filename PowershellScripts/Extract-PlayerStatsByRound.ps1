param (
    [string] $folderPath = 'fl:\PrevSeasonAnalysis\PlayerStats\CleanedPlayerStats',
    [string] $playerStatsFolderName = 'fl:\PrevSeasonAnalysis\PlayerStats'
)
$fileNames = Get-ChildItem $folderPath -filter *.csv | % { $_.Name } | sort-object
# join-string $fileNames -separator ',' | out-clipboard

$csvFiles = $fileNames | ? { $_ -match 'PlayerStats_Round(?<round>\d+)_(?<year>\d{4})_(?<month>\d{2})_(?<day>\d{2})\.csv' } | 
    select-object `
        @{n='Round';e={[int]::Parse($matches.Round)}},`
        @{n='FileName';e={$matches[0] }},`
        @{n='Year';e={$matches.year}},`
        @{n='Month';e={$matches.Month}},`
        @{n='Day';e={$matches.Day}}

$csvFilesByRound = $csvFiles `
    | select-object FileName,Round,Year,Month,Day,@{n='Date';e={new-object datetime -argumentList $_.Year,$_.Month,$_.Day}} `
    | sort-object Round

$csvFileContentsByRound = @{}
$csvFilesByRound | % {
    $round = $_.Round
    $playerStats = import-csv "$folderPath\$($_.FileName)"
    $csvFileContentsByRound[$round] = $playerStats
}
# $csvFileContentsByRound

$finalCsvFile = $csvFilesByRound[-1].FileName
...
0..21 | ? { $roundsWithPriceChanges.$_ } | Select-Object @{n='Round';e={$_}},@{n='PriceChanges';e={$roundsWithPriceChanges.$_}} `
    | export-csv "$playerStatsFolderName\RoundsWithPriceChanges.csv" -noTypeInformation
